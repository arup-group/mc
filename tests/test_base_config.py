...
def test_param_equality_same():
    test_config1 = _Config(path=env.test_xml_path)["planCalcScore"]['scoringParameters:default']["lateArrival"]
    test_config2 = _Config(path=env.test_xml_path)["planCalcScore"]['scoringParameters:default']["lateArrival"]
    assert test_config1 == test_config2


def test_paramset_equality_same():
    test_config1 = _Config(path=env.test_xml_path)["planCalcScore"]['scoringParameters:default']
    test_config2 = _Config(path=env.test_xml_path)["planCalcScore"]['scoringParameters:default']
    assert test_config1 == test_config2


def test_module_equality_same():
    test_config1 = _Config(path=env.test_xml_path)["planCalcScore"]
    test_config2 = _Config(path=env.test_xml_path)["planCalcScore"]
    assert test_config1 == test_config2


def test_config_equality_same():
    test_config1 = _Config(path=env.test_xml_path)
    test_config2 = _Config(path=env.test_xml_path)
    assert test_config1 == test_config2


def test_equality_not_same():
    test_config1 = _Config(path=env.test_xml_path)
    test_config2 = _Config(path=env.test_bad_config_path)
    assert not test_config1 == test_config2


def test_xml_and_json_test_configs_equal():
    test_config1 = _Config(path=env.test_xml_path)
    test_config2 = _Config(path=env.test_json_path)
    assert test_config1 == test_config2
...
def test_xml_read_json_write_read_consistency():
    ...
    test_config2 = _Config(path=env.test_temp_json_path)
    assert test_config1 == test_config2


def test_json_read_xml_write_read_consistency():
    test_config1 = _Config(path=env.test_json_path)
    test_config1.write_json(env.test_temp_json_path)
    test_config2 = _Config(path=env.test_temp_xml_path)
    assert test_config1 == test_config2
...
def test_module_dict_set():
    test_config_transit = _Config(path=env.test_bad_config_path)['transit']
    test_config = _Config(path=env.test_json_path)
...
def test_paramset_level_1_dict_set():
    test_config_score = _Config(path=env.test_bad_config_path)['planCalcScore']["scoringParameters:default"]
    test_config = _Config(path=env.test_json_path)
