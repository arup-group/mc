...
def test_set_cooling(config, total_iterations, start_index, step, new_fraction):
    assert (
        not config["strategy"]["fractionOfIterationsToDisableInnovation"]
        == new_fraction
    )
    autostep.set_cooling(
        config=config,
...
def test_set_default_behaviours(config, tmp_path):
    ...
    assert config["parallelEventHandling"]["eventsQueueSize"] == "270000000"
...
def test_autoset_input_paths(config):
    ...
    assert (
        config["transit"]["transitScheduleFile"]
        == "test/ing/output_transitSchedule.xml.gz"
    )
...
def test_fix_relative_home_input_paths_to_abs(test_path_config):
    ...
    assert (
        test_path_config["transit"]["transitScheduleFile"]
        == "~/test/schedule-merged.xml"
    )
...
def test_find_and_set_param(config):
    autostep.find_and_set_overrides(config, {"modeParams::car/constant": "-1.0"})
    assert (
        config["planCalcScore"]["scoringParameters::default"]["modeParams::car"][
            "constant"
        ]
        == "-1.0"
    )
    assert (
        config["planCalcScore"]["scoringParameters::unknown"]["modeParams::car"][
            "constant"
        ]
        == "-1.0"
    )
    assert (
        config["planCalcScore"]["scoringParameters::unknown"]["modeParams::bus"][
            "constant"
        ]
        == "0.0"
    )
...
def test_finding_and_setting_bad_param_leaves_config_unchanged(config):
    ...
    autostep.find_and_set_overrides(config, {"modeParams::*/horseback": "-1.0"})
    assert cnfg == config
...
def test_autostep_config_first_iteration(tmp_path):
    ...
    overrides_path = os.path.join(
        os.path.dirname(__file__), "test_data", "test_overrides.json"
    )
...
def test_nonexistent_optional_input_module_ignored_when_fixing_relative_paths():
    config_path = os.path.join(
        os.path.dirname(__file__), "test_data", "test_config_missing_all_vehicles.xml"
    )
    config = BaseConfig(config_path)
    assert "vehicles" not in config

    autostep.fix_relative_input_paths_to_abs(config, config_path)
...
def test_optional_vehicles_module_gets_relative_path_fixed():
    config_path = os.path.join(
        os.path.dirname(__file__), "test_data", "test_config.xml"
    )
    config = BaseConfig(config_path)
...
def test_autostep_config(tmp_path):
    ...
    assert config["network"]["inputNetworkFile"] == os.path.join(
        tmp_path, "10", "output_network.xml.gz"
    )
    assert config["plans"]["inputPlansFile"] == os.path.join(
        tmp_path, "10", "output_plans.xml.gz"
    )
    assert config["vehicles"]["vehiclesFile"] == os.path.join(
        tmp_path, "10", "output_vehicles.xml.gz"
    )
    assert config["transit"]["transitScheduleFile"] == os.path.join(
        tmp_path, "10", "output_transitSchedule.xml.gz"
    )
    assert config["transit"]["vehiclesFile"] == os.path.join(
        tmp_path, "10", "output_transitVehicles.xml.gz"
    )
    assert (
        config["planCalcScore"]["scoringParameters::default"]["modeParams::car"][
            "constant"
...
def test_step_lambda(tmp_path, fake_lambda_handler):
    event = {
        "orchestration": {
            ...
            "seed_matsim_config_path": os.path.join(
                "tests", "test_data", "test_config.xml"
            ),
            "start_index": "0",
            "total_iterations": "100",
            "step": "10",
    ...
    assert orchestration["biteration_matsim_config_path"] == os.path.join(
        tmp_path, "0", "matsim_config.xml"
    )
...
def test_stepping(tmp_path, fake_lambda_handler):
    ...
    for i in range(10):
        ...
        autostep.autostep_config(
            ...
            total_iterations=orchestration["total_iterations"],
            step=orchestration["step"],
            biteration_matsim_config_path=orchestration[
    ...
    for i in range(10, 30, 10):
        ...
        assert config["controler"]["outputDirectory"] == out_dir
        assert config["network"]["inputNetworkFile"] == os.path.join(
            tmp_path, str(i), "output_network.xml.gz"
        )
        assert config["plans"]["inputPlansFile"] == os.path.join(
            tmp_path, str(i), "output_plans.xml.gz"
        )
        assert config["vehicles"]["vehiclesFile"] == os.path.join(
            tmp_path, str(i), "output_vehicles.xml.gz"
        )
        assert config["transit"]["transitScheduleFile"] == os.path.join(
            tmp_path, str(i), "output_transitSchedule.xml.gz"
        )
        assert config["transit"]["vehiclesFile"] == os.path.join(
            tmp_path, str(i), "output_transitVehicles.xml.gz"
        )
        assert (
            config["planCalcScore"]["scoringParameters::default"]["modeParams::car"][
                "constant"
