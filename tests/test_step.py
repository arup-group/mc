...
def config():
    in_file = Path(
        os.path.join(os.path.dirname(__file__), "test_data", "test_config.xml")
    )
...
def test_set_input_paths(config):
    ...
    assert (
        config["plans"]["inputPersonAttributesFile"]
        == "test/ing/population_attributes.xml.gz"
    )
...
def test_find_and_set_param(config):
    step.find_and_set_overrides(config, {"modeParams::car/constant": "-1.0"})
    assert (
...
def test_find_and_set_bad_param(config):
    ...
    step.find_and_set_overrides(config, {"modeParams::*/horseback": "-1.0"})
    assert cnfg == config
...
def test_step_config(tmp_path):
    ...
    step.step_config(
        ...
        overrides=(
            ...
            "step",
            "999",
            "modeParams::car/constant",
            "-1.0",
            "scoringParameters::unknown/modeParams::bus/constant",
            "-1.0",
        ),
    )
