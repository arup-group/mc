    ...
    def __init__(self):
        ...
        for subpop in [
            ...
            for mode, mut_hr, mdr in zip(modes, mut_hrs, mdrs):
                ...
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"modeParams::{mode}"
                ]["marginalUtilityOfTraveling_util_hr"] = mut_hr
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"modeParams::{mode}"
                ]["monetaryDistanceRate"] = mdr
            ...
            for act, typ_dur, min_dur in zip(activities, typ_durs, min_durs):
                ...
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"activityParams::{act}"
                ]["typicalDuration"] = typ_dur
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"activityParams::{act}"
                ]["minimalDuration"] = min_dur
        ...
        for subpop in ["low income", "medium income", "high income", "freight"]:
            ...
            for strategy, weight in zip(strategies, weights):
                self["strategy"][f"strategysettings::{subpop}::{strategy}"][
                    "strategyName"
                ] = strategy
                self["strategy"][f"strategysettings::{subpop}::{strategy}"][
                    "subpopulation"
...
class BuildConfig(BaseConfig):
    ...
    def __init__(
        ...
        default_mode_scoring = {
            "car": {"mut_hr": "-6", "mdr": "-0.0002"},
            "pt": {"mut_hr": "-6", "mdr": "-0.0005"},
            "walk": {"mut_hr": "-12", "mdr": "0"},
            "access_walk": {"mut_hr": "-12", "mdr": "0"},
            "egress_walk": {"mut_hr": "-12", "mdr": "0"},
            "bike": {"mut_hr": "-12", "mdr": "0"},
        }

        default_activity_scoring = {
            "home": {"typ_dur": "12:00:00", "min_dur": "08:00:00"},
            "work": {"typ_dur": "08:30:00", "min_dur": "08:00:00"},
            "education": {"typ_dur": "08:00:00", "min_dur": "06:00:00"},
            "depo": {"typ_dur": "12:00:00", "min_dur": "08:00:00"},
            "delivery": {"typ_dur": "00:20:00", "min_dur": "00:10:00"},
            "other": {"typ_dur": "00:15:00", "min_dur": "00:10:00"},
        }
        ...
        for subpop in subpops:
            ...
            for mode in modes:
                ...
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"modeParams::{mode}"
                ]["marginalUtilityOfTraveling_util_hr"] = default_mode_scoring.get(
                    mode, default_mode_scoring["car"]
                )[
                    "mut_hr"
                ]
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"modeParams::{mode}"
                ]["monetaryDistanceRate"] = default_mode_scoring.get(
                    mode, default_mode_scoring["car"]
                )[
                    "mdr"
                ]
            ...
            for act in acts:
                ...
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"activityParams::{act}"
                ]["typicalDuration"] = default_activity_scoring.get(
                    mode, default_activity_scoring["other"]
                )[
                    "typ_dur"
                ]
                self["planCalcScore"][f"scoringParameters::{subpop}"][
                    f"activityParams::{act}"
                ]["minimalDuration"] = default_activity_scoring.get(
                    mode, default_activity_scoring["other"]
                )[
                    "min_dur"
                ]
        ...
        if "freight" in subpops:
            self["strategy"]["strategysettings::freight::SubtourModeChoice"][
                "weight"
            ] = "0"
